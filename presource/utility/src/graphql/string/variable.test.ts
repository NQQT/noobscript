import { createGraphQLVariableString } from '@presource/utility';
...
describe('graphQL Variable String', () => {
    test('Basic Variable String', () => {
        ...
        expect(createGraphQLVariableString({ name: { first: 'john', last: 'doe' } })).toBe(
    ...
    test('More commonly used as follow', () => {
        ...
        expect(createGraphQLVariableString({ name: '$hisname' })).toBe('(name:$hisname)');
    });
});
